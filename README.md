...
Return a parsed URL as per Node's `require("url").parse`. Validates to ensure it has a `protocol`, `hostname` and `port` as a minimum. `port` is validated to be numeric via `!isNaN(value)`.
